...
// ---------- 公共小工具 ----------
static inline int mod9(long long x){ x%=9; if(x<0) x+=9; return (int)x; }

// d^2 的十进制展开（最多两位）
static inline pair<int,int> sqSplit(int d){
    int sq=d*d;
    if(sq<10) return {sq, -1};
...
static inline int value_mod9_from_counts(const array<int,10>& cnt){
    long long s=0;
    for(int d=0; d<10; ++d) s += 1LL*d*cnt[d];
    return mod9(s);
}
...
int solve_brutal_string(long long n, long long m){
    // 拆初始 n 的十进制到向量
    if(n==0) return mod9(0 + 0); // 先不急，统一流程也可
    vector<int> cur;
    if(n==0) cur.push_back(0);
    while(n>0){ cur.push_back(n%10); n/=10; }
    ...
    for(long long t=1; t<=m; ++t){
        ...
        for(int d: cur){
            ...
            if(hi>=0) nxt.push_back(hi);
            if(lo>=0) nxt.push_back(lo);
        }
        cur.swap(nxt);
...
int solve_brutal_counts(long long n, long long m){
    ...
    if(n==0) cnt[0] = 1;
    while(n>0){ cnt[n%10] = mod9(cnt[n%10]+1); n/=10; }
    ...
    for(long long t=1; t<=m; ++t){
        ...
        for(int d=0; d<=9; ++d){
            ...
            if(!k) continue;
            auto [hi, lo] = sqSplit(d);
            if(hi>=0) nxt[hi] = (nxt[hi] + k) % 9;
            if(lo>=0) nxt[lo] = (nxt[lo] + k) % 9;
        }
        cnt = nxt;
...
int main(){
    ...
    while(T--){
        ...
        // 两个答案都算一遍（你也可以注释掉其一）
        int a = solve_brutal_string(n,m);  // 真·暴力
